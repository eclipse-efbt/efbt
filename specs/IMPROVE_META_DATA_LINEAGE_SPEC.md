...
Instead of filtering on union tables, report cells should filter directly on the relevant product-specific classes. The mapping is determined by the `INSTRMNT_TYP_PRDCT` filter value in each report cell.

### Implementation Strategy

#### 1. Product Mapping
- Use the `INSTRMNT_TYP_PRDCT` member from report cell combination items
- Map to product-specific classes using the configuration file:
  ...
  - **Mapping**: `INSTRMNT_TYP_PRDCT` → `Main Category` column → `slice name` column
...
2. Identify the `INSTRMNT_TYP_PRDCT` original member
...
- Add logic to identify relevant product-specific classes based on `INSTRMNT_TYP_PRDCT`
...
- Some `INSTRMNT_TYP_PRDCT` values map to multiple products
...
1. ✅ Analyzed existing report cell definitions and `INSTRMNT_TYP_PRDCT` usage patterns
2. ✅ Created comprehensive mapping between `INSTRMNT_TYP_PRDCT` values and product classes via CSV configuration
...
5. ✅ Updated filter generation to include `INSTRMNT_TYP_PRDCT` filters for completeness
...
The system now:
- Maps `INSTRMNT_TYP_PRDCT` values directly to product-specific classes using CSV configuration
- Generates report cells that filter directly on relevant product classes
- Includes redundant `INSTRMNT_TYP_PRDCT` filters for completeness and consistency
- Eliminates all union table overhead and intermediary wrapper classes
- Assumes all report cells have `INSTRMNT_TYP_PRDCT` values (no fallback needed)
