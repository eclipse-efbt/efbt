...
class RegDNAToDJango(object):
    ...
    def convert(self, context):
        ...
        if context.ldm_or_il == "ldm":
            RegDNAToDJango.createDjangoForPackage(
                self, context.ldm_entities_package, models_file, context
            )
            RegDNAToDJango.createDjangoAdminForPackage(
                self, context.ldm_entities_package, admin_file, context
            )
        else:
            RegDNAToDJango.createDjangoForPackage(
                self, context.il_tables_package, models_file, context
            )
            RegDNAToDJango.createDjangoAdminForPackage(
                self, context.il_tables_package, admin_file, context
            )
    ...
    def createDjangoForPackage(self, elpackage, output_file, context):
        ...
        for theImport in elpackage.imports:
            if not (theImport.importedNamespace.trim == "types.*"):
                output_file.write(
                    "from " + theImport.importedNamespace + " import *\r\n"
                )
        class_names_written = []
    ...
    def write_class_and_superclasses_in_correct_order(
        self, elclass, output_file, classes_written
    ):
        if elclass.name in classes_written:
            ...
            for elmember in elclass.eStructuralFeatures:
                if isinstance(elmember, ELAttribute):
                    if isinstance(elmember.eAttributeType, ELEnum):
                        output_file.write(
                            "\t"
                            + RegDNAToDJango.djangoChoices(
                                self, elmember.eAttributeType
                            )
                            + "\r\n"
                        )
                        output_file.write(
            ...
            if not long_name_exists:
                ...
                output_file.write(
                    "\t\t" + "verbose_name_plural = '" + elclass.name + "s'\r\n"
                )
