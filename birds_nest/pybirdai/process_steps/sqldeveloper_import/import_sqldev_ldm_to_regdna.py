...
class SQLDevLDMImport(object):
    ...
    def add_ldm_classes_to_package(self, context):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "ldm" + os.sep + "DM_Entities.csv"
        )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                ...
                if not header_skipped:
                    ...
                    if (num_supertype_entity_id == "") or (
                        num_supertype_entity_id is None
                    ):
    ...
    def import_disjoint_subtyping_information(self, context):
        ...
        entity_to_arc_dictionary = SQLDevLDMImport.get_entity_to_arc_dictionary(
            self, context, file_location
        )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                ...
                if not header_skipped:
                    ...
                    try:
                        num_of_siblings_arcs = (
                            len(entity_to_arc_dictionary[entity_name]) - 1
                        )
                    except KeyError:
                        pass
                    ...
                    if num_of_siblings_arcs > 0:
                        arc_class = None
                        try:
                            arc_class = context.arc_name_to_arc_class_map[
                                altered_arc_name
                            ]
                        except KeyError:
                            ...
                            context.arc_name_to_arc_class_map[altered_arc_name] = (
                                arc_class
                            )
                            ...
                            non_containment_reference.name = (
                                altered_arc_name + "_delegate"
                            )
                            ...
                            the_reference_annotation_directive = (
                                Utils.get_annotation_directive(
                                    source_class.eContainer(), "relationship_type"
                                )
                            )
                            the_reference_annotation.source = (
                                the_reference_annotation_directive
                            )
                            ...
                            mapentry.value = "true"
                            details.append(mapentry)
                            non_containment_reference.eAnnotations.append(
                                the_reference_annotation
                            )

                            arc_class.eStructuralFeatures.append(attribute)
                            the_identified_class_annotation = ELAnnotation()
                            the_identified_class_directive = (
                                Utils.get_annotation_directive(
                                    source_class.eContainer(), "relationship_type"
                                )
                            )
                            the_identified_class_annotation.source = (
                                the_identified_class_directive
                            )
                            ...
                            mapentry.value = (
                                source_class.name + "." + non_containment_reference.name
                            )
                            details.append(mapentry)
                            arc_class.eAnnotations.append(
                                the_identified_class_annotation
                            )
                        ...
                        context.arc_target_to_arc_map[
                            Utils.make_valid_id(target_entity_name)
                        ] = target_class
    ...
    def add_ldm_enums_to_package(self, context):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "ldm" + os.sep + "DM_Domains.csv"
        )
        header_skipped = False
        counter = 0
    ...
    def add_ldm_literals_to_enums(self, context):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "ldm" + os.sep + "DM_Domain_AVT.csv"
        )
        header_skipped = False
        counter = 0
                ...
                if not header_skipped:
                    ...
                    try:
                        ...
                        try:
                            ...
                            context.enum_literals_map[
                                the_enum.name + ":" + enum_literal.literal
                            ] = enum_literal
                        except KeyError:
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    rdbms_version = row[4]
                    datatype_id = row[0]
                    if (rdbms_type.strip() == "Oracle Database") and (
                        rdbms_version.strip() == "12cR2"
                    ):
                        native_type = row[2]
    ...
    def add_ldm_attributes_to_classes(self, context):
        ...
        file_location = (
            context.file_directory + os.sep + "ldm" + os.sep + "DM_Attributes.csv"
        )
        header_skipped = False
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    if len(preferred_abbreviation) == 0:
                        ...
                        print(
                            "no preferred abbreviation for attribute: " + attribute_name
                        )
                    ...
                    if attribute_kind == "Domain":
                        ...
                        if the_enum.name == "String":
                            ...
                            "Non_negative_monetary_amounts_with_2_decimals"
                        ):
                            attribute.name = the_attribute_name
                            attribute.eType = context.types.e_int
                            attribute.eAttributeType = context.types.e_int
                    ...
                    if attribute_kind == "Logical Type":
                        ...
                        try:
                            ...
                            attribute.eAttributeType = (
                                Utils.get_ecore_datatype_for_datatype(self)
                            )
    ...
    def add_ldm_relationships_between_classes(self, context):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "ldm" + os.sep + "DM_Relations.csv"
        )
        header_skipped = False
        with open(file_location, encoding="utf-8") as csvfile:
            filereader = csv.reader(csvfile, delimiter=",", quotechar='"')
                ...
                if not header_skipped:
                    ...
                    if (not (the_class is None)) and (not (target_class is None)):
                        ...
                        if target_optional == "N":
                            ...
                            the_associated_class_directive = (
                                Utils.get_annotation_directive(
                                    the_class.eContainer(), "relationship_type"
                                )
                            )
                            the_associated_class_annotation.source = (
                                the_associated_class_directive
                            )
                            ...
                            details.append(mapentry)
                            target_class.eAnnotations.append(
                                the_associated_class_annotation
                            )
    ...
    def mark_root_class_as_entity_group_annotation(self, context):
        for the_class in context.classes_map.values():
            ultimate_superclass = SQLDevLDMImport.get_ultimate_superclass(
                self, context, the_class
            )
            if not (ultimate_superclass == the_class):
                the_entity_annotation = Utils.get_annotation_with_source(
                    the_class, "entity_hierarchy"
                )
    ...
    def get_ultimate_superclass(self, context, the_class):
        ...
        if len(the_class.eSuperTypes) > 0:
            return_class = SQLDevLDMImport.get_ultimate_superclass(
                self, context, the_class.eSuperTypes[0]
            )
    ...
    def get_delegate_class(self, context, the_class):
        ...
        for a_class in context.classes_map.values():
            for reference in a_class.eStructuralFeatures:
                if (
                    reference.name.endswith("_delegate")
                ) and reference.eType == the_class:
                    return a_class
            ...
            if isinstance(classifier, ELClass):
                ...
                for feature in classifier.eStructuralFeatures:
                    if isinstance(feature, ELAttribute):
                        if SQLDevLDMImport.attribute_exists_in_any_superclass(
                            self, classifier, feature, context
                        ):
    ...
    def attribute_exists_in_any_superclass(self, el_class, attribute, context):
        ...
        if len(el_class.eSuperTypes) > 0:
            ...
            if return_value == False:
                return SQLDevLDMImport.attribute_exists_in_any_superclass(
                    self, super_class, attribute, context
                )
            else:
                return True
