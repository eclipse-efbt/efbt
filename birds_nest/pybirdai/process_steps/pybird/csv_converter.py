...
class CSVConverter:
    def persist_object_as_csv(theObject, useLongNames):
        ...
        csvString = CSVConverter.createCSVStringForTable(
            theObject, useLongNames, table_name
        )
        try:
            if useLongNames:
    ...
    def createCSVStringForTable(theObject, useLongNames, table_name):
        ...
        for o in object_list:
            if not headerCreated:
                csvString = csvString + CSVConverter.createCSVHeaderStringForRow(
                    o, django_model
                )
                headerCreated = True
                csvString = csvString + CSVConverter.createCSVStringForRow(
                    o, useLongNames, django_model
                )
    ...
    def get_contained_objects(theObject):
        ...
        try:
            ...
            instance_members = [
                ...
                if not callable(getattr(theObject.__class__, member))
                and not member.startswith("__")
            ]

            for member in instance_members:
                if not (member.endswith("Table")) and isinstance(
                    getattr(theObject.__class__, member), list
                ):
    ...
    def createCSVStringForRow(theObject, useLongNames, django_model):
        ...
        if django_model:
            references = [
                ...
                if not callable(getattr(theObject.__class__, method))
            ...
            operations = [
                method
                for method in dir(theObject.__class__)
                if callable(getattr(theObject.__class__, method))
                and not method.startswith("__")
            ]
        ...
        if is_reference:
            ...
            if upperbound == 1:
                ...
                references = [
                    ...
                    if not callable(getattr(eStructuralFeature.__class__, method))
                    and not method.startswith("__")
                ]
                ...
                for eStructuralFeature2 in references:
                    if eStructuralFeature2 == idattr:
                        attributeValue = getattr(
                            referencedItem.__class__, eStructuralFeature2
                        )
                        if attributeValue:
