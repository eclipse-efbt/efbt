...
class CreateExecutableFilters:
    def create_executable_filters(self, context, sdd_context):
        ...
        file = open(
            sdd_context.output_directory
            + os.sep
            + "generated_python_filters"
            + os.sep
            + "report_cells.py",
            "a",
            encoding="utf-8",
        ...
        report_html_file.write(
            "<a href=\"{% url 'pybirdai:home'%}\">Back to the PyBIRD AI Home Page</a>\n"
        )
        ...
        for (
            ...
            for combination in combination_list:
                if combination.combination_id.metric:
                    ...
                    file.write(
                        "\t\t\ttotal += item."
                        + combination.combination_id.metric.name
                        + "()\n"
                    )
                    file.write("\t\treturn total\n")
                    file.write("\tdef calc_referenced_items(self):\n")
                    file.write(
                        "\t\titems = self." + cube_id + "_Table." + cube_id + "s\n"
                    )
    ...
    def get_leaf_node_codes(self, sdd_context, member, member_hierarchy):
        ...
        if member is not None:
            return_list = (
                CreateExecutableFilters.get_member_list_considering_hierarchies(
                    self, sdd_context, member, member_hierarchy
                )
            )
        return return_list
    ...
    def get_literal_list_considering_hierarchies(
        ...
        for (
            domain,
            hierarchy_list,
        ) in sdd_context.domain_to_hierarchy_dictionary.items():
            if domain is None:
            ...
            for item in child_members:
                ...
                if not (literal is None):
                    if not (literal in literal_list):
                        is_node = CreateExecutableFilters.is_member_a_node(
                            self, sdd_context, literal
                        )
                        if not (is_node):
    ...
    def find_member_node(self, sdd_context, member_id, hierarchy):
        try:
            return sdd_context.member_hierarchy_node_dictionary[
                hierarchy + ":" + member_id.member_id
            ]
        except:
            pass
    ...
    def prepare_node_dictionaries_and_lists(self, sdd_context):
        ...
        for node in sdd_context.member_hierarchy_node_dictionary.values():
            if not (node.parent_member_id is None) and not (
                node.parent_member_id == ""
            ):
                sdd_context.members_that_are_nodes.append(node.parent_member_id)
                member_plus_hierarchy = (
                    node.parent_member_id.member_id
                    + ":"
                ...
                if not (
                    member_plus_hierarchy
                    in sdd_context.member_plus_hierarchy_to_child_literals.keys()
                ):
                    initial_member_list = []
                    ...
                    member_list = sdd_context.member_plus_hierarchy_to_child_literals[
                        member_plus_hierarchy
                    ]
        ...
        for hierarchy in sdd_context.member_hierarchy_dictionary.values():
            try:
                hierarchy_list = sdd_context.domain_to_hierarchy_dictionary[
                    hierarchy.domain_id
                ]
                ...
                hierarchy_list = []
                hierarchy_list.append(hierarchy)
                sdd_context.domain_to_hierarchy_dictionary[hierarchy.domain_id] = (
                    hierarchy_list
                )

    def get_member_list_considering_hierarchies(
        self, sdd_context, member, member_hierarchy
    ):
        return_list = []
        is_node = CreateExecutableFilters.is_member_a_node(self, sdd_context, member)
            ...
            for (
                ...
                if domain is None:
                    ...
                    for hierarchy in hierarchy_list:
                        hierarchy_id = hierarchy.member_hierarchy_id
                        member_list = []
                        CreateExecutableFilters.get_member_list_considering_hierarchy(
                            self, sdd_context, member, hierarchy_id, member_list
                        )
                        return_list.extend(member_list)

        return return_list

    def get_member_list_considering_hierarchy(
        self, sdd_context, member, hierarchy, member_list
    ):
        key = member.member_id + ":" + hierarchy
        child_members = []
        try:
            ...
            for item in child_members:
                CreateExecutableFilters.get_member_list_considering_hierarchy(
                    self, sdd_context, item, hierarchy, member_list
                )
        except KeyError:
            pass
