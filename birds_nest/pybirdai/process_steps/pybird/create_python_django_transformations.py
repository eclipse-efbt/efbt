...
class CreatePythonTransformations:
    ...
    def create_output_classes(sdd_context):
        ...
        for report_id, cube_links in sdd_context.cube_link_to_foreign_cube_map.items():
            ...
            file.write("\nclass " + report_id + ":\n")
            file.write(
                "\tunionOfLayers = None #  " + report_id + "_UnionItem  unionOfLayers\n"
            )
            cube_structure_items = []
            try:
                cube_structure_items = sdd_context.rol_cube_structure_item_dictionary[
                    report_id + "_cube_structure"
                ]
            except KeyError:
            ...
            for cube_structure_item in cube_structure_items:
                ...
                if domain == "String":
                    ...
                    file.write(
                        "\tdef " + variable.variable_id + "(self) -> datetime:\n"
                    )
                elif domain == "Float":
                    ...
                    file.write("\tdef " + variable.variable_id + "(self) -> str:\n")
                    file.write(
                        "\t\t''' return string from " + domain + " enumeration '''\n"
                    )

                file.write(
                    "\t\treturn self.unionOfLayers." + variable.variable_id + "()\n"
                )
                file.write("\n")
            file.write("\n")
            ...
            file.write(
                "\tdef  calc_" + report_id + "s(self) -> list[" + report_id + "] :\n"
            )
            ...
            file.write(
                "\t\tself." + report_id + "s.extend(self.calc_" + report_id + "s())\n"
            )
    ...
    def create_slice_classes(sdd_context):
        for report_id, cube_links in sdd_context.cube_link_to_foreign_cube_map.items():
            ...
            file.write(
                "\t" + report_id + "_UnionItems = [] # " + report_id + "_UnionItem []\n"
            )
            join_ids_added = []
            for (
                join_for_report_id,
                cube_links,
            ) in sdd_context.cube_link_to_join_for_report_id_map.items():
                ...
                for cube_link in cube_links:
                    ...
                    if the_report_id == report_id:
                        ...
                        try:
                            ...
                            print(
                                f"No cube structure item links for cube_link: {cube_link.cube_link_id}"
                            )
                        primary_cubes_added = []
            ...
            for (
                ...
                if report_and_join[0] == report_id:
                    ...
                    file.write(
                        "\tdef calc_" + join_id.replace(" ", "_") + "s(self) :\n"
                    )
