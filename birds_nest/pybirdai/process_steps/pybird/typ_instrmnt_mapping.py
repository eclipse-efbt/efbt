...
class TypInstrmntMapper:
    ...
    def __init__(self, framework="FINREP_REF"):
        """Initialize mapper with specified framework.
    ...
    def load_mappings(self):
        """Load condition to slice mappings from CSV file for the specified framework"""
        base_dir = settings.BASE_DIR
        mapping_file = os.path.join(base_dir, 'resources', 'joins_configuration',
                                   f'join_for_product_to_reference_category_{self.framework}.csv')
        ...
        if os.path.exists(mapping_file):
            ...
            print(f"Loaded {len(self.condition_to_slices)} mappings from {self.framework}")
        else:
