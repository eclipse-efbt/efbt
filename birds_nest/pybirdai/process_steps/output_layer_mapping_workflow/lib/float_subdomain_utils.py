...
def check_float_subdomain_status() -> Dict[str, Any]:
    """
    Check the current status of Float subdomain and MTRC variable.

    Returns:
        Dict with status information
    """
    status = {
        'float_domain_exists': False,
        'float_subdomain_exists': False,
        'mtrc_exists': False,
        'mtrc_domain': None,
        'mtrc_subdomain': None
    }
    ...
    # Check Float domain
    status['float_domain_exists'] = DOMAIN.objects.filter(domain_id='Float').exists()

    # Check Float subdomain
    float_subdomain = SUBDOMAIN.objects.filter(subdomain_id='Float').first()
    status['float_subdomain_exists'] = float_subdomain is not None

    # Check MTRC variable
    mtrc = VARIABLE.objects.filter(variable_id='MTRC').first()
    if mtrc:
        status['mtrc_exists'] = True
        status['mtrc_domain'] = mtrc.domain_id.domain_id if mtrc.domain_id else None
        status['mtrc_subdomain'] = mtrc.subdomain_id.subdomain_id if mtrc.subdomain_id else None
...
if __name__ == "__main__":
    ...
    print("Checking Float subdomain status...")
    status = check_float_subdomain_status()
    print(f"Status: {status}")

    print("\nEnsuring Float subdomain for MTRC...")
    result = ensure_float_subdomain_for_mtrc()
