...
class JoinsMetaDataCreator:
    ...
    def do_stuff_and_prepare_context(self, context: Any, sdd_context: Any):
        ...
        context.category_to_combinations = {
            category: {item.combination_id for item in COMBINATION_ITEM.objects.all().filter(member_id__member_id = category)}
            for category in all_main_categories
        }
        context.category_to_ci = {
            category: { combination_id: {
    ...
    def operation_exists_in_cell_for_report_with_category(self, context: Any, sdd_context: Any, output_item: Any, category: str, report_template: str) -> Tuple[bool, dict]:
        """
        ...
        """
        report_combinations = {el.combination_id for el in sdd_context.combination_to_rol_cube_map.get(report_template, [])}
    ...
    def find_variables_with_same_domain_then_name(self,context:Any, sdd_context: Any, output_item: Any, input_entity: Any) -> List[Any]:
        """
        Find variables with the same domain and then name as the output item.
        ...
        if target_domain and target_domain.domain_id and target_domain.domain_id not in IGNORED_DOMAINS:
            # Find variables with the same domain
            related_variables = []
            for csi in field_list:
