...
class ImportDatabaseToSDDModel(object):
    ...
    def create_all_mapping_definitions(self, context):
        ...
        for mapping_definition in MAPPING_DEFINITION.objects.all():
            context.mapping_definition_dictionary[mapping_definition.mapping_id] = (
                mapping_definition
            )
    ...
    def create_all_rol_cube_structures(self, context):
        ...
        for rol_cube_structure in CUBE_STRUCTURE.objects.all():
            context.rol_cube_structure_dictionary[
                rol_cube_structure.cube_structure_id
            ] = rol_cube_structure
    ...
    def create_all_mapping_to_cubes(self, context):
        ...
        for mapping_to_cube in MAPPING_TO_CUBE.objects.all():
            try:
                ...
                context.mapping_to_cube_dictionary[mapping_to_cube.cubeMapping] = [
                    mapping_to_cube
                ]
    ...
    def create_all_variables(self, context):
        ...
        for variable in VARIABLE.objects.all():
            ...
            context.variable_to_primary_concept_map[variable.variable_id] = (
                variable.primary_concept
            )
    ...
    def create_all_nonref_member_hierarchies(self, context):
        ...
        for hierarchy in MEMBER_HIERARCHY.objects.all():
            context.member_hierarchy_dictionary[hierarchy.member_hierarchy_id] = (
                hierarchy
            )
    ...
    def create_all_nonref_member_hierarchies_nodes(self, context):
        ...
        for hierarchy_node in MEMBER_HIERARCHY_NODE.objects.all():
            ...
            context.member_hierarchy_node_dictionary[
                hierarchy_node.member_hierarchy_id.member_hierarchy_id
                + ":"
                + member_name
            ] = hierarchy_node
    ...
    def create_axis_ordinates(self, context):
        ...
        for axis_ordinate in AXIS_ORDINATE.objects.all():
            context.axis_ordinate_dictionary[axis_ordinate.axis_ordinate_id] = (
                axis_ordinate
            )
    ...
    def create_table_cells(self, context):
        ...
        for table_cell in TABLE_CELL.objects.all():
            ...
            try:
                ...
                context.table_to_table_cell_dictionary[table_cell.table_id] = (
                    table_cell_list
                )
    ...
    def create_cell_positions(self, context):
        ...
        for cell_position in CELL_POSITION.objects.all():
            try:
                ...
                context.cell_positions_dictionary[cell_position.cell_id.cell_id] = [
                    cell_position
                ]
    ...
    def create_member_mappings(self, context):
        ...
        for member_mapping in MEMBER_MAPPING.objects.all():
            context.member_mapping_dictionary[member_mapping.member_mapping_id] = (
                member_mapping
            )
...
>>>>>>> 6fc3086b (>efbt-1430 : Add Subdomain and VariableSet and MemberLink to SDD domain model + format)
    ...
    def create_cube_links(self, context):
        ...
        for cube_link in CUBE_LINK.objects.all():
            ...
            try:
                context.cube_link_to_foreign_cube_map[foreign_cube.cube_id].append(
                    cube_link
                )
            except KeyError:
                context.cube_link_to_foreign_cube_map[foreign_cube.cube_id] = [
                    cube_link
                ]
            join_identifier = cube_link.join_identifier
            try:
                context.cube_link_to_join_identifier_map[join_identifier].append(
                    cube_link
                )
            except KeyError:
            ...
            try:
                context.cube_link_to_join_for_report_id_map[join_for_report_id].append(
                    cube_link
                )
            except KeyError:
                context.cube_link_to_join_for_report_id_map[join_for_report_id] = [
                    cube_link
                ]
    ...
    def create_cube_structure_item_links(self, context):
        ...
        for cube_structure_item_link in CUBE_STRUCTURE_ITEM_LINK.objects.all():
            ...
            try:
                ...
                ].append(cube_structure_item_link)
            except KeyError:
                context.cube_structure_item_link_to_cube_link_map[
                    cube_link.cube_link_id
                ] = [cube_structure_item_link]
