...
class ImportWebsiteToSDDModel(object):
    ...
    def import_hierarchies_from_sdd(self, sdd_context):
        ...
        ImportWebsiteToSDDModel.create_all_parent_members_with_children_locally(
            self, sdd_context
        )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    maintenance_agency.maintenance_agency_id = (
                        ImportWebsiteToSDDModel.replace_dots(self, id)
                    )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    framework.framework_id = ImportWebsiteToSDDModel.replace_dots(
                        self, id
                    )
    ...
    def create_all_domains(self, context, ref):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "technical_export" + os.sep + "domain.csv"
        )
        header_skipped = False
        domains_to_create = []
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    if include:
                        ...
                        if ref:
                            maintenance_agency = (
                                ImportWebsiteToSDDModel.find_maintenance_agency_with_id(
                        ...
                        domain.domain_id = ImportWebsiteToSDDModel.replace_dots(
                            self, domain_id
                        )
    ...
    def create_all_members(self, context, ref):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "technical_export" + os.sep + "member.csv"
        )
        header_skipped = False
        members_to_create = []
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    maintenence_agency = row[
                        ColumnIndexes().member_member_maintenence_agency
                    ]
                    ...
                    if include:
                        ...
                        member.member_id = ImportWebsiteToSDDModel.replace_dots(
                            self, member_id
                        )
                        member.code = code
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    maintenence_agency = row[
                        ColumnIndexes().variable_variable_maintenence_agency
                    ]
                    ...
                    if include:
                        ...
                        if not ((primary_concept == "") or (primary_concept == None)):
                            context.variable_to_primary_concept_map[variable_id] = (
                                primary_concept
                            )
    ...
    def create_all_parent_members_with_children_locally(self, context):
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    parent_member_id = row[
                        ColumnIndexes().member_hierarchy_node_parent_member_id
                    ]
                    member_id = row[ColumnIndexes().member_hierarchy_node_member_id]
                    hierarchy_id = row[
                        ColumnIndexes().member_hierarchy_node_hierarchy_id
                    ]
                    hierarchy = ImportWebsiteToSDDModel.find_member_hierarchy_with_id(
                        self, hierarchy_id, context
                    )
                    if hierarchy:
                        domain = hierarchy.domain_id
                        if not (parent_member_id is None) and not (
                            parent_member_id == ""
                        ):
        ...
        for parent_member_id, member_id, domain in parent_members_child_triples:
            if member_id in parent_members:
                if (
                    not (parent_member_id in context.members_that_are_nodes.keys())
                    and not (
                        parent_member_id in context.nonref_member_dictionary.keys()
                    )
                    and not (parent_member_id in context.ref_member_dictionary.keys())
                ):
                    parent_member = MEMBER(
                        name=ImportWebsiteToSDDModel.replace_dots(
                            self, parent_member_id
                        )
                    )
                    parent_member.member_id = ImportWebsiteToSDDModel.replace_dots(
                        self, parent_member_id
                    )
                    parent_member.maintenance_agency_id = (
                        ImportWebsiteToSDDModel.find_maintenance_agency_with_id(
                            self, context, "NODE"
                        )
                    )
                ...
                if not (member is None):
                    if (
                        ...
                        and not (
                            parent_member_id in context.ref_member_dictionary.keys()
                        )
                    ):
                        parent_member = MEMBER(
                            name=ImportWebsiteToSDDModel.replace_dots(
        ...
        if context.save_sdd_to_db and parent_members_to_create:
            MEMBER.objects.bulk_create(parent_members_to_create, batch_size=1000)

        for missing_member in missing_children:
            print(
                f"Parent member {missing_member[0]} has missing child {missing_member[1]}"
            )
    ...
    def create_all_member_hierarchies(self, context):
        """
        Import all member hierarchies from CSV file using bulk create
        """
        missing_domains = []
        file_location = (
            context.file_directory
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    maintenance_agency_id = row[
                        ColumnIndexes().member_hierarchy_maintenance_agency
                    ]
                    ...
                    if domain is None:
                        ...
                        if not (id in context.member_hierarchy_dictionary.keys()):
        ...
        if context.save_sdd_to_db and hierarchies_to_create:
            MEMBER_HIERARCHY.objects.bulk_create(hierarchies_to_create, batch_size=1000)
    ...
    def create_all_member_hierarchies_nodes(self, context):
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    if hierarchy is None:
                        ...
                        member = (
                            ImportWebsiteToSDDModel.find_member_with_id_for_hierarchy(
                                self, member_id, hierarchy, context
                            )
                        )
                        if member is None:
                            print(
                                f"Member {member_id} not found in the database for hierarchy {hierarchy_id}"
                            )
                            missing_members.append((hierarchy_id, member_id))
                        else:
                            parent_member = ImportWebsiteToSDDModel.find_member_with_id(
                                self, parent_member_id, context
                            )
        ...
        ImportWebsiteToSDDModel.save_missing_hierarchies_to_csv(
            context, missing_hierarchies
        )
    ...
    def create_report_tables(self, context):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "technical_export" + os.sep + "table.csv"
        )
        header_skipped = False
        tables_to_create = []
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    maintenance_agency_id = row[
                        ColumnIndexes().table_maintenance_agency_id
                    ]
                    ...
                    table.table_id = ImportWebsiteToSDDModel.replace_dots(
                        self, table_id
                    )
    ...
    def create_axis(self, context):
        ...
        """
        file_location = (
            context.file_directory + os.sep + "technical_export" + os.sep + "axis.csv"
        )
        header_skipped = False
        axes_to_create = []
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    axis.table_id = ImportWebsiteToSDDModel.find_table_with_id(
                        self, context, axis_table_id
                    )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    axis_ordinate_description = row[
                        ColumnIndexes().axis_ordinate_description
                    ]

                    axis_ordinate = AXIS_ORDINATE(
                        name=ImportWebsiteToSDDModel.replace_dots(
                            self, axis_ordinate_id
                        )
                    )
                    axis_ordinate.axis_ordinate_id = (
                        ImportWebsiteToSDDModel.replace_dots(self, axis_ordinate_id)
                    )
                    ...
                    axis_ordinate.axis_id = ImportWebsiteToSDDModel.find_axis_with_id(
                        self,
                        context,
                        ImportWebsiteToSDDModel.replace_dots(
                            self, axis_ordinate_axis_id
                        ),
                    )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    axis_ordinate_id = row[
                        ColumnIndexes().ordinate_item_axis_ordinate_id
                    ]
                    variable_id = row[ColumnIndexes().ordinate_item_variable_id]
                    member_id = row[ColumnIndexes().ordinate_item_member_id]
                    member_hierarchy_id = row[
                        ColumnIndexes().ordinate_item_member_hierarchy_id
                    ]
                    starting_member_id = row[
                        ColumnIndexes().ordinate_item_starting_member_id
                    ]
                    is_starting_member_included = row[
                        ColumnIndexes().ordinate_item_is_starting_member_included
                    ]

                    ordinate_item = ORDINATE_ITEM()
                    ordinate_item.axis_ordinate_id = (
                        ImportWebsiteToSDDModel.find_axis_ordinate_with_id(
                            self, context, axis_ordinate_id
                        )
                    )
                    ordinate_item.variable_id = (
                        ImportWebsiteToSDDModel.find_variable_with_id(
                            self, context, variable_id
                        )
                    )
                    ordinate_item.member_id = (
                        ImportWebsiteToSDDModel.find_member_with_id(
                            self, member_id, context
                        )
                    )
                    ordinate_item.member_hierarchy_id = (
                        ImportWebsiteToSDDModel.find_member_hierarchy_with_id(
                            self, member_hierarchy_id, context
                        )
                    )
                    ordinate_item.starting_member_id = (
                        ImportWebsiteToSDDModel.find_member_with_id(
                            self, starting_member_id, context
                        )
                    )
                    ordinate_item.is_starting_member_included = (
                    ...
                    try:
                        ...
                        context.axis_ordinate_to_ordinate_items_map[
                            axis_ordinate_id
                        ] = [ordinate_item]
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    table_cell_combination_id = row[
                        ColumnIndexes().table_cell_combination_id
                    ]
                    ...
                    if table_cell_cell_id.endswith("_REF"):
                        table_cell = TABLE_CELL(
                            name=ImportWebsiteToSDDModel.replace_dots(
                                self, table_cell_cell_id
                            )
                        )
                        table_cell.cell_id = ImportWebsiteToSDDModel.replace_dots(
                            self, table_cell_cell_id
                        )
                        table_cell.table_id = (
                            ImportWebsiteToSDDModel.find_table_with_id(
                                self,
                                context,
                                ImportWebsiteToSDDModel.replace_dots(
                                    self, table_cell_table_id
                                ),
                            )
                        )
                        ...
                        table_cell_list = (
                            context.table_to_table_cell_dictionary.setdefault(
                                table_cell.table_id, []
                            )
                        )
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    cell_positions_axis_ordinate_id = row[
                        ColumnIndexes().cell_positions_axis_ordinate_id
                    ]
                    ...
                    if cell_positions_cell_id.endswith("_REF"):
                        ...
                        cell_position.cell_id = (
                            ImportWebsiteToSDDModel.find_table_cell_with_id(
                                self,
                                context,
                                ImportWebsiteToSDDModel.replace_dots(
                                    self, cell_positions_cell_id
                                ),
                            )
                        )
                        ...
                        cell_positions_list = (
                            context.cell_positions_dictionary.setdefault(
                                cell_positions_cell_id, []
                            )
                        )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    maintenance_agency_id = row[
                        ColumnIndexes().member_map_maintenance_agency_id
                    ]
                    member_mapping_id = row[
                        ColumnIndexes().member_map_member_mapping_id
                    ]
                    ...
                    if not member_mapping_id.startswith("SHS_"):
                        ...
                        member_mapping.maintenance_agency_id = (
                            ImportWebsiteToSDDModel.find_maintenance_agency_with_id(
                                self, context, maintenance_agency_id
                            )
                        )
                        ...
                        context.member_mapping_dictionary[member_mapping_id] = (
                            member_mapping
                        )

        if context.save_sdd_to_db and member_mappings_to_create:
            MEMBER_MAPPING.objects.bulk_create(
                member_mappings_to_create, batch_size=1000
            )
    ...
    def create_all_member_mappings_items(self, context):
                ...
                if not header_skipped:
                    ...
                    if not member_mapping_id.startswith("SHS_"):
                        ...
                        if member is None or variable is None:
                            ...
                            member_mapping_item.member_mapping_id = (
                                ImportWebsiteToSDDModel.find_member_mapping_with_id(
        ...
        if context.save_sdd_to_db and member_mapping_items_to_create:
            MEMBER_MAPPING_ITEM.objects.bulk_create(
                member_mapping_items_to_create, batch_size=1000
            )
        ...
        ImportWebsiteToSDDModel.save_missing_mapping_members_to_csv(
            context, missing_members
        )
    ...
    def save_missing_mapping_variables_to_csv(context, missing_variables):
        filename = (
            context.output_directory
            + os.sep
            + "generated_mapping_warnings"
            + os.sep
            + "mappings_missing_variables.csv"
        )
        with open(filename, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
    ...
    def save_missing_mapping_members_to_csv(context, missing_members):
        ...
        ImportWebsiteToSDDModel.create_mappings_warnings_summary(
            context, missing_members
        )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    mapping_definition_name = row[
                        ColumnIndexes().mapping_definition_name
                    ]
                    mapping_definition_code = row[
                        ColumnIndexes().mapping_definition_code
                    ]
                    mapping_definition_mapping_type = row[
                        ColumnIndexes().mapping_definition_mapping_type
                    ]
                    mapping_definition_member_mapping_id = row[
                        ColumnIndexes().mapping_definition_member_mapping_id
                    ]
                    mapping_definition_variable_mapping_id = row[
                        ColumnIndexes().mapping_definition_variable_mapping_id
                    ]

                    if not mapping_definition_id.startswith("SHS_"):
                        mapping_definition = MAPPING_DEFINITION(
                            mapping_id=mapping_definition_id,
                            name=mapping_definition_name,
                            code=mapping_definition_code,
                            mapping_type=mapping_definition_mapping_type,
                            member_mapping_id=ImportWebsiteToSDDModel.find_member_mapping_with_id(
                        ...
                        context.mapping_definition_dictionary[mapping_definition_id] = (
                            mapping_definition
                        )

        if context.save_sdd_to_db and mapping_definitions_to_create:
            MAPPING_DEFINITION.objects.bulk_create(
                mapping_definitions_to_create, batch_size=1000
            )
    ...
    def create_all_mapping_to_cubes(self, context):
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    mapping_to_cube_mapping_id = row[
                        ColumnIndexes().mapping_to_cube_mapping_id
                    ]
                    mapping_to_cube_cube_mapping_id = row[
                        ColumnIndexes().mapping_to_cube_cube_mapping_id
                    ]
                    mapping_to_cube_valid_from = row[
                        ColumnIndexes().mapping_to_cube_valid_from
                    ]
                    mapping_to_cube_valid_to = row[
                        ColumnIndexes().mapping_to_cube_valid_to
                    ]

                    if not mapping_to_cube_mapping_id.startswith("M_SHS"):
                        mapping_to_cube = MAPPING_TO_CUBE(
                            mapping=ImportWebsiteToSDDModel.find_mapping_definition_with_id(
                                self, context, mapping_to_cube_mapping_id
                            ),
                            cubeMapping=ImportWebsiteToSDDModel.replace_dots(
                                self, mapping_to_cube_cube_mapping_id
                            ),
                        ...
                        mapping_to_cube_list = (
                            context.mapping_to_cube_dictionary.setdefault(
        ...
        if context.save_sdd_to_db and mapping_to_cubes_to_create:
            MAPPING_TO_CUBE.objects.bulk_create(
                mapping_to_cubes_to_create, batch_size=1000
            )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    variable_mapping_id = row[
                        ColumnIndexes().variable_mapping_variable_mapping_id
                    ]
                    variable_mapping_maintenance_agency_id = row[
                        ColumnIndexes().variable_mapping_maintenance_agency_id
                    ]
                    ...
                    if not (
                        variable_mapping_id
                        in context.variable_mapping_dictionary.keys()
                    ):
                        if not variable_mapping_id.startswith("SHS_"):
                            variable_mapping = VARIABLE_MAPPING(
                                ...
                                maintenance_agency_id=ImportWebsiteToSDDModel.find_maintenance_agency_with_id(
                                    self,
                                    context,
                                    variable_mapping_maintenance_agency_id,
                                ),
                                code=variable_mapping_code,
                                name=variable_mapping_name,
                            )

                            variable_mappings_to_create.append(variable_mapping)
                            context.variable_mapping_dictionary[variable_mapping_id] = (
                                variable_mapping
                            )

        if context.save_sdd_to_db and variable_mappings_to_create:
            VARIABLE_MAPPING.objects.bulk_create(
                variable_mappings_to_create, batch_size=1000
            )
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    variable_mapping_item_variable_mapping_id = row[
                        ColumnIndexes().varaible_mapping_item_variable_mapping_id
                    ]
                    variable_mapping_item_variable_id = row[
                        ColumnIndexes().variable_mapping_item_variable_id
                    ]
                    variable_mapping_item_is_source = row[
                        ColumnIndexes().variable_mapping_item_is_source
                    ]
                    variable_mapping_item_valid_from = row[
                        ColumnIndexes().variable_mapping_item_valid_from
                    ]
                    variable_mapping_item_valid_to = row[
                        ColumnIndexes().variable_mapping_item_valid_to
                    ]
                    ...
                    if not variable_mapping_item_variable_mapping_id.startswith("SHS_"):
                        ...
                        if variable is None:
                            missing_variables.append(
                                (
                                    variable_mapping_item_variable_id,
                                    variable_mapping_item_variable_mapping_id,
                                    variable_mapping_item_valid_to,
                                )
                            )
                        else:
                            variable_mapping_item = VARIABLE_MAPPING_ITEM(
                                ...
                                is_source=variable_mapping_item_is_source,
                                valid_from=variable_mapping_item_valid_from,
                                valid_to=variable_mapping_item_valid_to,
                            )

                            if variable_mapping_item.variable is not None:
                                variable_mapping_items_to_create.append(
                                    variable_mapping_item
                                )

                                variable_mapping_items_list = (
                                    context.variable_mapping_item_dictionary.setdefault(
                                        variable_mapping_item_variable_mapping_id, []
                                    )
                                )
                                variable_mapping_items_list.append(
                                    variable_mapping_item
                                )
                            else:
                                print(
                                    f"No Variable mapping item variable id: {variable_mapping_item_variable_id}"
                                )

        if context.save_sdd_to_db and variable_mapping_items_to_create:
            VARIABLE_MAPPING_ITEM.objects.bulk_create(
                variable_mapping_items_to_create, batch_size=1000
            )

        for missing_variable in missing_variables:
            print(f"Missing Variable {missing_variable}")

        ImportWebsiteToSDDModel.save_missing_mapping_variables_to_csv(
    ...
    def delete_hierarchy_warnings_files(self, context):
        ...
        hierarchy_warnings_dir = os.path.join(
            base_dir, "results", "generated_hierarchy_warnings"
        )
    ...
    def delete_mapping_warnings_files(self, context):
        base_dir = settings.BASE_DIR
        mapping_warnings_dir = os.path.join(
            base_dir, "results", "generated_mapping_warnings"
        )
