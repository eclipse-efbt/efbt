            ...
            if member_id in parent_members:
                ...
                if not (member is None):
                    if not (parent_member_id in context.members_that_are_nodes.keys()):
                        ...
                        if context.save_sdd_to_db:
                            parent_member.save()
                            context.members_that_are_nodes[parent_member_id] = (
                                parent_member
                            )
                else:
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    member_hierarchy_valid_from = row[
                        ColumnIndexes().ordinate_item_member_hierarchy_valid_from
                    ]
        ...
        with open(file_location, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    table_cell_system_data_code = row[
                        ColumnIndexes().table_cell_system_data_code
                    ]
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    if cell_positions_cell_id.endswith("_REF"):
                        ...
                        try:
                            cell_positions_list = context.cell_positions_dictionary[
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    if not mapping_definition_id.startswith("SHS_"):
                        ...
                        mapping_definition.mapping_type = (
                            mapping_definition_mapping_type
                        )
                        mapping_definition.member_mapping_id = (
                            ImportWebsiteToSDDModel.find_member_mapping_with_id(
                                self, context, mapping_definition_member_mapping_id
                            )
                        )
                        mapping_definition.variable_mapping_id = (
                            ImportWebsiteToSDDModel.find_variable_mapping_with_id(
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    if not mapping_to_cube_mapping_id.startswith("M_SHS"):
                        ...
                        mapping_to_cube.mapping = (
                            ImportWebsiteToSDDModel.find_mapping_definition_with_id(
        ...
        with open(filelocation, encoding="utf-8") as csvfile:
            ...
            for row in filereader:
                if not header_skipped:
                    ...
                    variable_mapping.maintenance_agency_id = (
                        ImportWebsiteToSDDModel.find_maintenance_agency_with_id(
                ...
                if not header_skipped:
                    ...
                    if not variable_mapping_item_variable_mapping_id.startswith("SHS_"):
                        ...
                        if variable is None:
                            ...
                            variable_mapping_item.variable_mapping_id = (
                                ImportWebsiteToSDDModel.find_variable_mapping_with_id(
                                    self,
                                    context,
                                    variable_mapping_item_variable_mapping_id,
                            ...
                            variable_mapping_item.is_source = (
                                variable_mapping_item_is_source
                            )
                            variable_mapping_item.valid_from = (
                                variable_mapping_item_valid_from
                            )
                            variable_mapping_item.valid_to = (
                                variable_mapping_item_valid_to
                            )
