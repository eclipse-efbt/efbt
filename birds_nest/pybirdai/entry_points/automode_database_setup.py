...
class RunAutomodeDatabaseSetup(AppConfig):
    ...
    def run_post_setup_operations(self):
        ...
        try:
            ...
            logger.info("This will trigger Django restart - user will need to run migrations after restart")
            ...
            logger.info("Django will restart now. After restart, user needs to run migrations.")
            return {
                'success': True,
                'step': 1,
    ...
    def _run_django_system_command(self, base_dir):
        ...
        logger.warning("Using legacy _run_django_system_command - consider using _run_migrations_in_subprocess instead")
        ...
        try:
            ...
            logger.info("Running makemigrations command programmatically (fast mode)...")
            makemig_start = time.time()
            
            # Capture output but suppress verbosity for speed
            out = io.StringIO()
            call_command('makemigrations', 'pybirdai', stdout=out, stderr=out, verbosity=0)
            output = out.getvalue()
            
            makemig_time = time.time() - makemig_start
            logger.info(f"Makemigrations completed in {makemig_time:.2f}s - Output: {output.strip()}")
            ...
            migrate_time = time.time() - migrate_start
            logger.info(f"Migrate completed in {migrate_time:.2f}s - Output: {output.strip()}")

            total_time = time.time() - start_time
            logger.info(f"All Django commands completed in {total_time:.2f}s total. Django restart may occur now.")
