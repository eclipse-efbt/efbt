...
"""
Setup service for initial repository cloning and file organization.

This service is DESTRUCTIVE - it deletes existing directories before copying.
Use ONLY during initial automode setup, NOT during workflow execution.

For workflow execution (non-destructive updates), use MirrorRepoService instead.
"""
...
class SetupRepoService:
    """
    Service class for cloning a repository and setting up files according to the mapping configuration.

    WARNING: This service is DESTRUCTIVE - it deletes existing directories before copying.
    Use ONLY during initial automode setup.
    ...
    def _copy_tree_with_filter(self, source_dir: str, target_dir: str, filter_func):
        """
        Copy a directory tree with file filtering.

        Args:
            source_dir: Source directory path
            target_dir: Target directory path
            filter_func: Function that takes a relative file path and returns True if it should be copied
        """
        os.makedirs(target_dir, exist_ok=True)
        ...
        for root, dirs, files in os.walk(source_dir):
            ...
            for file_name in files:
                if rel_root == '.':
                    ...
                    rel_file = os.path.join(rel_root, file_name)

                if filter_func(rel_file):
                    source_file = os.path.join(root, file_name)
                    target_file = os.path.join(target_root, file_name)
                    shutil.copy2(source_file, target_file)
                    logger.debug(f"Copied (passed filter): {rel_file}")
                else:
                    logger.debug(f"Skipped (filtered out): {rel_file}")
    ...
    def setup_files(self, destination_path: str = "FreeBIRD", pipeline: str = None):
        """
        Organize and copy files from the extracted repository according to REPO_MAPPING configuration.

        WARNING: This method is DESTRUCTIVE - it deletes existing target directories!
        ...
        Args:
            ...
            pipeline (str, optional): Pipeline name to filter filter_code by type.
                                     If provided, only imports 'datasets' for ancrdt
                                     or 'templates' for dpm/main pipelines.
        """
        start_time = time.time()
        logger.info(f"Starting file setup from {destination_path}")
        logger.warning("DESTRUCTIVE OPERATION: This will delete and replace target directories")
        ...
        for source_folder, target_mappings in REPO_MAPPING.items():
            ...
            # DESTRUCTIVE: Remove existing target folder and copy fresh from source
            ...
            # Special handling for filter_code with pipeline filtering
            is_filter_code = 'filter_code' in source_folder
            if is_filter_code and pipeline:
                # Use pipeline-aware copy that filters by type (datasets/templates)
                pipeline_filter = create_pipeline_filter(pipeline)
                self._copy_tree_with_filter(source_folder_path, target_folder, pipeline_filter)
                logger.info(f"Copied filter_code directory with {pipeline} pipeline filter: {source_folder_path} -> {target_folder}")
            else:
...
CloneRepoService = SetupRepoService
...
def main():
    ...
    """
    start_time = time.time()
    logger.info("Starting SetupRepoService execution")

    # Create service instance and execute the complete workflow
    service = SetupRepoService()
    ...
    end_time = time.time()
    logger.info(f"SetupRepoService execution completed in {end_time - start_time:.2f} seconds")
